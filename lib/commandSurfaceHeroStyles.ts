...
export function splitHeroLogoWellLinkClass(size: HeroSize): string {
  return `logo-well relative overflow-hidden inline-flex flex-col items-center justify-center self-start h-fit w-fit max-w-full ${splitHeroLogoWellSizingClass(size)} transition-transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-aurora-primary/50 rounded-2xl border border-aurora-border/60`;
}

...
export function ForYouHeaderRailsSkeleton() {
  return (
    <div className="mt-4 space-y-3" aria-hidden>
      <div className="h-3 w-44 rounded bg-aurora-surface-hover animate-pulse" />
...
export function ForYouSectionsSkeleton() {
  return (
    <div className="space-y-10 sm:space-y-12" aria-busy="true" aria-label="Loading suggestions">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-48 rounded-2xl bg-aurora-surface-hover animate-pulse" />
          ))}
        </div>
      </div>
    </div>
